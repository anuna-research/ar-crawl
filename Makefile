...
# Generate version info from git tag
version-info:
	@echo "Generating version info from git..."
	@VERSION=$$(git describe --tags --always 2>/dev/null || echo "0.0.0-dev"); \
	echo "#lang racket/base" > src/version-info.rkt; \
	echo "" >> src/version-info.rkt; \
	echo ";; version-info.rkt - Auto-generated version information" >> src/version-info.rkt; \
	echo ";; This file is regenerated by \`make binary\` or \`make dist\` from git tags." >> src/version-info.rkt; \
	echo ";; Do not edit manually - changes will be overwritten during build." >> src/version-info.rkt; \
	echo "" >> src/version-info.rkt; \
	echo "(provide AR-CRAWL-VERSION)" >> src/version-info.rkt; \
	echo "" >> src/version-info.rkt; \
	echo ";; Version from git tag (updated at build time)" >> src/version-info.rkt; \
	echo "(define AR-CRAWL-VERSION \"$$VERSION\")" >> src/version-info.rkt; \
	echo "Version set to: $$VERSION"

# Binary build
binary: install version-info
